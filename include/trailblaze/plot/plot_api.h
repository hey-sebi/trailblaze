...
inline void RenderScene(const Scene& sc, Renderer& r, int width_px = 1000,
                        int height_px = 800, std::string title = {})
{
  ...
  r.SetAxisEqual(true);
  for (const auto& pl : sc.polylines) r.Draw(pl);
  for (const auto& pg : sc.polygons) r.Draw(pg);
  for (const auto& ar : sc.arrows) r.Draw(ar);
  for (const auto& tx : sc.texts) r.Draw(tx);
  r.EndFigure();
}

template <typename State>
inline void PlotR2Path(const trailblaze::Path<State>& path, Renderer& r,
                       std::string title = "Path (R^2)")
{
  static_assert(trailblaze::has_xy_v<State>,
                "PlotR2Path requires a state with members x and y.");
  Scene sc;
...
template <typename State>
inline void PlotSE2Path(const trailblaze::Path<State>& path, Renderer& r,
