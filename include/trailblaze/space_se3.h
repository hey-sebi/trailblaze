...
// -------------------- Accessors --------------------
namespace access {
inline double& x(SE3& s) noexcept { return s.pos.x; }
inline double& y(SE3& s) noexcept { return s.pos.y; }
inline double& z(SE3& s) noexcept { return s.pos.z; }
inline double x(const SE3& s) noexcept { return s.pos.x; }
inline double y(const SE3& s) noexcept { return s.pos.y; }
inline double z(const SE3& s) noexcept { return s.pos.z; }

inline double& qx(SE3& s) noexcept { return s.ori.qx; }
inline double& qy(SE3& s) noexcept { return s.ori.qy; }
inline double& qz(SE3& s) noexcept { return s.ori.qz; }
inline double& qw(SE3& s) noexcept { return s.ori.qw; }
inline double qx(const SE3& s) noexcept { return s.ori.qx; }
inline double qy(const SE3& s) noexcept { return s.ori.qy; }
inline double qz(const SE3& s) noexcept { return s.ori.qz; }
inline double qw(const SE3& s) noexcept { return s.ori.qw; }
}  // namespace access
...
struct MetricSe3Weighted {
  ...
  double operator()(const SE3& a, const SE3& b) const {
    ...
    const double dot = a.ori.qx * b.ori.qx + a.ori.qy * b.ori.qy +
                       a.ori.qz * b.ori.qz + a.ori.qw * b.ori.qw;
